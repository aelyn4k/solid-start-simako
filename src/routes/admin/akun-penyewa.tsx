...
export default function AdminTenantAccountsRoute() {
  ...
  const tenantUsers = createMemo(() =>
    users.filter((user) => user.role === "penyewa"),
  );
  ...
  return (
    <Sidebar
      title="Akun Penyewa"
      subtitle="Daftar akun penyewa kost yang terdaftar di SIMAKO."
    >
      <section class="dashboard-card overflow-hidden">
        <div class="flex flex-wrap items-center justify-between gap-3 border-b border-[var(--divider)] p-4">
          <div>
            <h2 class="ui-heading text-lg font-bold">Akun Penyewa Kost</h2>
            <p class="dashboard-muted mt-1 text-sm">Search dan filter status akun penyewa kost.</p>
          </div>
        ...
        <div class="grid gap-3 border-b border-[var(--divider)] p-4 lg:grid-cols-[1fr_0.35fr]">
          ...
          <FilterSelect<StatusFilter>
            value={statusFilter()}
            ariaLabel="Filter status akun penyewa"
            options={[
        ...
        {filteredUsers().length === 0 && (
          <div class="p-5">
            <div class="rounded-xl border border-dashed border-[var(--surface-border)] bg-[var(--control-bg)] p-6 text-center text-sm text-[rgb(var(--text-muted-rgb))]">
              Tidak ada akun penyewa yang cocok dengan filter.
            </div>
          </div>
        )}
        <div class="p-4">
          <Pagination
            ariaLabel="Pagination akun penyewa"
            page={page()}
