...
const handleRoomPointerMove: JSX.EventHandler<HTMLElement, PointerEvent> = (event) => {
...
const handleRoomPointerLeave: JSX.EventHandler<HTMLElement, PointerEvent> = (event) => {
  const card = event.currentTarget;
...
const RoomCard: Component<{ room: Room }> = (props) => (
  ...
  >
    <div class="overflow-hidden">
      <img src={props.room.image} alt={props.room.name} class="room-card-image h-48 w-full object-cover" />
    </div>
    <div class="relative z-10 flex flex-1 flex-col p-5">
      <div class="flex items-start justify-between gap-3">
        <div>
          <p class="ui-muted text-xs font-semibold uppercase tracking-[0.16em]">{props.room.type}</p>
      ...
      <A href={`/room/${props.room.id}`} class="btn-secondary mt-6 w-full px-4 py-2 text-sm">
        Lihat Detail Kamar
      </A>
    </div>
    ...
    <main class="pb-16">
      ...
      <section class="layout-shell py-14 md:py-16">
        <div class="mb-8 flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <p class="ui-text mt-2">Pilihan kamar terbaik dengan informasi yang jelas dan status terkini.</p>
          </div>
          <A href="/search" class="text-sm font-semibold text-red-400 transition hover:text-red-300">
            Lihat Semua Kamar
          </A>
        </div>
      ...
      <section class="layout-shell py-12 md:py-16">
        <div class="mb-8 text-center">
          <h2 class="ui-heading text-3xl font-bold md:text-4xl">Fitur Unggulan SIMAKO</h2>
          <p class="ui-text mx-auto mt-3 max-w-2xl">
        ...
        <div class="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-4">
          {features.map((feature) => (
            <article class="surface-card h-full p-6 transition duration-300 hover:-translate-y-1 hover:border-red-400/50">
              ...
              <p class="ui-text mt-3 text-sm leading-7">{feature.description}</p>
            </article>
          ))}
        </div>
      </section>
            ...
            <div>
              ...
              <div class="mt-6 space-y-4 text-sm">
                ...
                <div class="flex items-start gap-3">
                  ...
                  <p class="ui-title">Layanan Pengelola: Senin - Minggu, 08.00 - 21.00 WIB</p>
                </div>
              </div>
