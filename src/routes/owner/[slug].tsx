...
export default function OwnerDetailPage() {
  ...
  const ownerName = createMemo(
    () => ownerRooms()[0]?.ownerName ?? "Pemilik Kost",
  );
  const availableCount = createMemo(
    () => ownerRooms().filter((room) => room.status === "Tersedia").length,
  );
      ...
      {ownerRooms().length > 0 ? (
        <>
              ...
              <div class="mt-7 grid gap-8 lg:grid-cols-[1fr_0.8fr] lg:items-start">
                <div>
                  ...
                  <p class="ui-lead mt-5 max-w-2xl leading-8">
                    Lihat daftar kamar dari {ownerName()}, status ketersediaan,
                    fasilitas utama, dan detail harga sewa sebelum menghubungi
                    pengelola.
                  </p>
                </div>
          ...
          <section class="layout-shell py-12">
            <div class="mb-7">
              <h2 class="ui-heading text-3xl font-bold">
                Daftar Kamar {ownerName()}
              </h2>
              <p class="ui-text mt-2">
                Halaman ini membantu mesin pencari memahami kamar yang dimiliki
                oleh setiap pemilik kost.
              </p>
            </div>
        ...
        <section class="layout-shell flex min-h-[62vh] items-center justify-center py-16">
          <div class="surface-card max-w-xl p-8 text-center">
            <h1 class="ui-heading text-3xl font-bold">
              Pemilik Kost Tidak Ditemukan
            </h1>
            <p class="ui-text mt-3">
              Data pemilik kost untuk slug {params.slug} tidak tersedia.
            </p>
